...
echo "Max Latitude: $maxlat\n";
echo "Min Latitude: $minlat\n";
...
User Avatar
03 Jul
It looks like you've shared a comprehensive HTML and PHP script that integrates Leaflet for mapping and uses PDO for database interactions. Let's focus on the PHP part where you are querying the database and fetching the results.
Here's a revised version of your PHP code that uses prepared statements and parameter binding with PDO:
```php
<!DOCTYPE html>
<!-- Leaflet is the primary mapping used here:
Primary maping program, also uses poiMarkers.php, objMarkers.php and stationMarkers.php -->
...
//$q = intval($_GET["NetID"]);
$q = 11001;
// We need the min & max latitude to determine if we want to pull data from poiMarkers.php
// This should be changed to min and max longitude or the Americas vs. Europe etc.
$sql = "SELECT MAX(latitude) as maxlat,
MIN(latitude) as minlat,
MAX(longitude) as maxlon,
MIN(longitude) as minlon
FROM NetLog
WHERE netID = :netID AND latitude <> ''";
try {
$stmt = $db_found->prepare($sql);
...
User Avatar
03 Jul
update the previous code as I requested please.
...
User Avatar
03 Jul
Sure, let's update your PHP code to use PDO with prepared statements and parameter binding. This will help improve security and maintainability. Here's the updated version of your PHP script:
```php
<!DOCTYPE html>
<!-- Leaflet is the primary mapping used here:
Primary maping program, also uses poiMarkers.php, objMarkers.php and stationMarkers.php -->
