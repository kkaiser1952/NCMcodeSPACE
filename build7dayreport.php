...
// Pre-assign default values to variables
$OpenCall = '';
$CloseCall = '';
$cscount = 0;
$netCnt = 0;
...
$sql = $db_found->prepare("
    SELECT
        COUNT(callsign) AS all_callsigns,
    ...
    FROM
        NetLog
    WHERE
        DATE(logdate) >= DATE_SUB(UTC_DATE(), INTERVAL 7 DAY);
");
$sql->execute();
...
if (!empty($result)) {
    ...
    echo '<h1 style="margin-left:300;">' . $title . '</h1>
        ...
        </div>' // this tick closes the echo at the $title line 
    ;
    } else {
