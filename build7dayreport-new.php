...
SELECT 
    ...
    CASE WHEN nl.logdate <> '0000-00-00 00:00:00' THEN nl.logdate
            ELSE (SELECT MAX(logdate) FROM NetLog) END AS logdate,
    nl.netcall,
    nl.stations,
    nl.pb,
    nl.testnet,
    ...
    CASE 
        ...
        WHEN nl.logdate = '0000-00-00 00:00:00' THEN TIME_TO_SEC((SELECT max(logdate) FROM NetLog))
    ...
    foreach ($result as $rowIndex => $row) {
      ...
      //  if ($row[netID] == 9685 ) { echo $row[netID] . ': LCTcss: ' . $LCTcss . ' CCss: ' . $CCss . ' FNcss: ' . $FNcss . ' THEcss: ' . $THEcss ;}        
        ...
        $dayOfWeek = date('l', strtotime($date));        
            ...
            $options = [
                "Map Net" => "https://net-control.space/map.php?NetID="    . $netID,
                "ICS-214" => "https://net-control.space/ics214.php?NetID=" . $netID,
                "ICS-209" => "https://net-control.space/ics309.php?NetID=" . $netID,
                "ICS-205" => "https://net-control.space/ics205A.php?NetID=". $netID,
                "Time Line" => "https://net-control.space/HzTimeline.php?NetID=" . $netID
                // https://net-control.space/buildUniqueCallList.php
            ...
            foreach ($row as $column => $columnValue) {
                if ($column === 'netID_count' OR $column === 'pb' OR $column === 'testnet' OR $column === 'PBcss' OR $column === 'LCTcss' OR $column === 'TNcss' OR $column === 'CCss' OR $column === 'Volunteer_Time' OR $column === 'FNcss' OR $column === 'SNcss') {
                    ...
                    if ($column === 'netID') {
                        ...
                        echo '<option value="https://net-control.space/map.php?NetID=' . $netID . '">Map Net</option>';
                        echo '<option value="https://net-control.space/ics214.php?NetID=' . $netID . '">ICS-214</option>';
                        echo '<option value="https://net-control.space/ics309.php?NetID=' . $netID . '">ICS-309</option>';
                        echo '<option value="https://net-control.space/ics205A.php?NetID=' . $netID .
                            '">ICS-205A</option>';
                        echo '<option value="https://net-control.space/HzTimeline.php?NetID=' . $netID .
                            '">TimeLine</option>';
