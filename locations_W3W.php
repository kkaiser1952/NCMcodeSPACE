<?php
// locations_W3W.php
// Designed to work much like its counterpart locations_APRS.php but with W3W as input by the logger.
// It is called by the fetch() in NetManager-W3W-APRS.js
// UPDATED: 2024-09-14
...
try {
    $aprs_call = isset($_GET["aprs_call"]) ? htmlspecialchars(filter_input(INPUT_GET, 'aprs_call', FILTER_UNSAFE_RAW)) : '';
    ...
    $cs1 = isset($_GET["cs1"]) ? htmlspecialchars(filter_input(INPUT_GET, 'cs1', FILTER_UNSAFE_RAW)) : '';
    $nid = isset($_GET["nid"]) ? filter_input(INPUT_GET, 'nid', FILTER_SANITIZE_NUMBER_INT) : '';
    $objName = isset($_GET["objName"]) ? htmlspecialchars(filter_input(INPUT_GET, 'objName', FILTER_UNSAFE_RAW)) : '';
    $W3Wcomment = isset($_GET["comment"]) ? htmlspecialchars(filter_input(INPUT_GET, 'comment', FILTER_UNSAFE_RAW)) : '';
    $what3words = isset($_GET["w3wfield"]) ? htmlspecialchars(filter_input(INPUT_GET, 'w3wfield', FILTER_UNSAFE_RAW)) : '';
    ...
    $deltax = 'LOC&#916;:W3W ' . $objName . ' : ' . $W3Wcomment . ' : ' . $what3words . ' : ' . $crossroads . ' : (' . $thislatlng . ')';
