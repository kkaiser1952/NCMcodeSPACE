...
// Use this program to compare NCM.stations with netcontrolcp_fcc_amateur.en and to 
...
SELECT n.callsign as callsign, 
       ...
       f.last  as last,
       f.city  as city,
       (SELECT MAX(fccid) FROM netcontrolcp_fcc_amateur.en WHERE callsign = n.callsign) as maxfccid,        
       CONCAT_WS(' ', f.address1, f.city, f.state, f.zip) as address
  FROM ncm.stations n
      ,netcontrolcp_fcc_amateur.en f
 WHERE n.callsign = f.callsign
  AND (SELECT MAX(fccid) FROM netcontrolcp_fcc_amateur.en WHERE callsign = n.callsign) 
  AND f.fccid > n.fccid 
  AND n.id < 38000
