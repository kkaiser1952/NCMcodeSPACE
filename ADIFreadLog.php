<?php
    ...
    //
    //  <call:6>KN4POG      <qso_date_off:8>20210117    <qso_date:8>20210117 
    //  <time_on:6>015315   <time_off:6>015315          <band:3>80m 
    //  <freq:8>3.574300    <mode:3>FT8                 <rst_sent:3>+06 
    //  <rst_rcvd:3>-05     <app_rumlog_qsl:1>W         <qsl_rcvd:1>N 
    //  <qsl_sent:1>R       <state:2>NC                 <dxcc:3>291 
    //  <cqz:1>5            <ituz:1>8                   <cont:2>NA 
    //  <name:5>Allen       <qth:9>Jamestown            <comment:25>FT8  Sent: +06  Rcvd: -05
    //  <gridsquare:10>FM05AX87JO                       <cnty:11>NC,GUILFORD 
    //  <tx_pwr:2>50        <app_rumlog_power:3>50w     <eqsl_qsl_rcvd:1>N 
    //  <eqsl_qsl_sent:1>N  <lotw_qsl_rcvd:1>N          <lotw_qsl_sent:1>N 
    //  <pfx:3>KN4          <clublog_qso_upload_status:1>Y 
    //  <app_rumlog_colorcode:1>0 <eor>
    ...
    function processCabrilloFile ($file, &$qso_count, $connection, $dxcallsign)
    {
        ...
        while ($s = fgets ($file,1024))
        {
            ...
            $query = "INSERT INTO myQSOs.qsos SET id = 0, " .
    ...
    function readADIFQSO ($string, &$qso_data, &$band_found, &$freq_found, &$comment)
    {
        ...
        // Read Comment
        if ($s = stristr($string,"<comment"))
        {
            $values = sscanf ($s, "<comment:%d>%s ", $length,$qso_data['comment']);

            if ($values != 2)
                sscanf ($s, "<comment:%d:%c>%s ", $length,$dummy,$qso_data['comment']);
        }
    ...
    function processADIFFile ($file, &$qso_count, $connection, $string, $dxcallsign)
    {
        ...
        readADIFQSO ($string, &$qso_data, &$band_found, &$freq_found, &$comment);
    ...
    if (is_uploaded_file ($temp_name))
    {
        ...
        $query = "SELECT callsign from myQSOs.station where id=$dxcallsign";
        ...
        $query = "INSERT INTO myQSOs.logfiles SET id = 0, " .
