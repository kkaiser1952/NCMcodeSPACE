...
<head>
    ...
    <meta http-equiv="refresh" content="5400; URL=https://net-control.space/help.php" >
...
"; // END OF header
	...
	$sql = ("
		SELECT  id, COUNT(*) AS call_count, callsign, Fname, Lname, phone, email, 
		     ...
		     /*   SUM(timeonduty) as time */
          FROM `NetLog` 
         WHERE netID <> 0
           AND id <> 0
           AND callsign <> 'NONHAM'
           AND Fname NOT LIKE '%(%'
           AND callsign NOT IN('BLIND','CARROLL','FSQCALL','NR0AD','OTHER',
                'TECHGUYS','W0KCN','W0KCK','KCABARC','CREW2273','KCHEART','MESN',
                'NCMO','K0ERC','K2BSA','TE0ST','W0TE','ORCA','W0ERH','K0HAM','KS0JC',
                'LCARES','K0ESM','KCMECC','BCARES''W0AU','WX9LOT','HAWK100','NWS','EVENT',
                'LBR','CMBR','CCARC','MAARS','LHHM','N6HEW','SKYWARN','JCARES','SCST','WINKC',
                'NA1WJ','FOW','BBCR','W0BU','W4ABP','WB4GQX')
